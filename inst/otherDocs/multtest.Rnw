% \VignetteIndexEntry{xmulttest Tutorial}
...
\title{\bf Bioconductor's xmulttest package}
...
% library(tools)
% Rnwfile<- file.path("/home/sandrine/CVS_stuff/madman/Rpacks/xmulttest/inst/doc",
%                     "xmulttest.Rnw") 
...
The {\tt xmulttest} package contains a collection of functions for
...
tutorial for using the {\tt xmulttest} package. For a detailed
introduction to multiple testing consult the document {\tt
  xmulttest.intro} in the {\tt inst/doc} directory of the package. See
also \cite{Shaffer95} and  \cite{Dudoit&Shaffer02} for a review of
multiple testing procedures and complete references.\\ 

{\bf Multiple testing procedures implemented in {\tt xmulttest}.}
The {\tt xmulttest} package implements multiple testing procedures for
...
%<<eval=TRUE, echo=TRUE>>=
<<>>=
library(xmulttest, verbose=FALSE)
data(golub)
@
